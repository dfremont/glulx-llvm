...
/*
    This version of nanoprintf has been slightly modified for use with the
    Glulx backend for LLVM by Daniel Fremont.
*/
...
NPF_VISIBILITY int npf_parse_format_spec(char const *format,
...
#if NANOPRINTF_USE_FLOAT_FORMAT_SPECIFIERS == 1
NPF_VISIBILITY int npf_fsplit_abs(float f, uint32_t *out_int_part,
...
enum {
    ...
    NPF_FRACTION_BIN_DIGITS = 32,
    NPF_MAX_FRACTION_DEC_DIGITS = 8
};

int npf_fsplit_abs(float f, uint32_t *out_int_part, uint32_t *out_frac_part,
    ...
    */
    ...
    if (exponent >= (32 - NPF_MANTISSA_BITS)) {
        return 0;
    }
                ...
                switch (fs.conv_spec) {
                    ...
                    case NPF_FMT_SPEC_CONV_POINTER: { /* 'p' */
                        cbuf_len = npf_utoa_rev(
                            cbuf, (npf_uint_t)(uintptr_t)va_arg(vlist, void *),
